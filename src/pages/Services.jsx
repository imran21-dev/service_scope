...
const Services = () => {
  ...
  const [allServicesCount, setAllServiceCount] = useState([])
  ...
  const count = allServicesCount.length
  ...
  useEffect(() => {
    ...
    axios.get('https://service-scope-server.vercel.app/all-service-count')
    .then(res => setAllServiceCount(res.data))
    axios
      .get(`https://service-scope-server.vercel.app/all-services?page=${page - 1}&size=${itemsPerPage}`)
      .then((res) => {
        setAllServices(res.data);
        complete();
        setSkeletonTime(false);
      });
  }, [complete,page, demoLoad, start]);
  ...
  useEffect(() => {
    ...
    setSkeletonTime(true);
    axiosSecure
      .get(`/filter-services?category=${selectedCategory.label}`)
      .then((res) => {
        setAllServices(res.data);
        complete();
        setSkeletonTime(false);
  ...
  const handleSearch = (e) => {
    ...
    start();
    setSkeletonTime(true);
    axiosSecure.get(`/search-services/?keyword=${keyword}`).then((res) => {
      setSkeletonTime(false);
      complete();
      setAllServices(res.data);
      setCross(true);
      ...
      <div className="md:w-10/12  w-full px-3 lg:px-0 md:mx-auto md:py-10 z-0 relative">
        ...
        <div className="grid lg:grid-cols-3 gap-3 lg:gap-6 items-center relative ">
          <h1 className="text-lg md:text-xl font-semibold">
            All Services {allServicesCount.length}
          </h1>

          <div className=" flex items-center gap-2 relative">
            <form
              onSubmit={handleSearch}
              className="border border-pColor/50 rounded-full items-center overflow-hidden flex w-full"
            >
              {cross ? (
                <button
                  className=""
        ...
        {sleketonTime ? (
          ...
          <div className="flex items-center flex-col justify-center h-screen w-full absolute -z-10 left-0 top-0">
            <Lottie
              loop={false}
              className="w-2/12"
